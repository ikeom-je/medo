...
| 実装・テスト | **Codex** | 実装計画のTaskに沿ったコード実装、テストコードの作成、大量生成(スキャフォールド・網羅的テスト生成・マイグレーション)、相互レビュー(Claude作の生成物) |
| 調査 | **agy(antigravity)** | Web検索(市場・国策・業界動向・技術情報)、長いドキュメント/コード全体の読み込みが要る調査、ダイジェスト作成、相互レビュー(Claude作の生成物) |
...
- **agyのGemini quotaが切れた場合**: agy自体が動くならagyの代替モデル(Claude系)に切り替えて調査・レビューを継続する。agy自体が利用不可の場合、調査はClaudeが自身の検索ツールで代替し、レビューは実施可能なレビュアーで行う。いずれもコミット本文・PR本文にその旨を記録する
- 振り分けの基準に迷ったら: 「意思決定・検証・裁定を伴うか」→ Claude、「手順が決まっているコード作成か」→ Codex、「外部情報の収集・大量読み込みか」→ agy
