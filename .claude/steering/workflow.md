...
| 実装・テストコード作成 | **Codex** | 実装計画のTaskに沿ったコード実装、テストコードの作成、大量生成(スキャフォールド・網羅的テスト生成・マイグレーション)、相互レビュー(Claude作の生成物) |
| 調査・情報抽出 | **agy(antigravity)** | Web検索(市場・国策・業界動向・技術情報)、マルチモーダル資料(PDF・スライド・画像)からの情報抽出、長いドキュメント/コード全体の読み込みが要る調査・ダイジェスト作成、GCP環境の調査(Cloud Logging・Vertex AI Search等)、相互レビュー(Claude作の生成物) |
...
- 振り分けの基準に迷ったら: 作業を「意思決定・検証・裁定 / 手順が決まっているコード作成 / 外部情報の収集・大量読み込み / 資料生成」のどの作業領域かで捉え、担当表の該当行に従う。どの行にも該当しなければ担当表に新しい行を足す判断をユーザーに確認する
