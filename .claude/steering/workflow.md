...
| 調査 | **agy(antigravity)** | Web検索(市場・国策・業界動向・技術情報)、マルチモーダル資料(PDF・スライド・画像)からの情報抽出、長いドキュメント/コード全体の読み込みが要る調査・ダイジェスト作成、GCP環境の調査(Cloud Logging・Vertex AI Search等)、Skillの「Geminiホスト」としての実行・eval(`generated_by`比較)、相互レビュー(Claude作の生成物) |
