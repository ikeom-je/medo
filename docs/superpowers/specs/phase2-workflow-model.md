...
**artifact束縛の「現在の対象」は `(type, slide_kind)` で決める**。`slides` は討議用と最終提案の2種類があり、typeだけで最新版を決めると最終提案スライドを保存した瞬間に「現在の slides」が入れ替わり、討議用スライド束縛の `expression_safety` が失効する。討議用スライド自体はストアに残っているが、**投影上の現在対象から外れる**ため、確認済みの項目が未確認へ戻る。
...
**決裁者が複数いる場合は1名の `agreed` で条件4を充足する**(`refs` には未取得の決裁者を全員返す)。決裁者として登録されるのは「決裁に関与しうる人」であり、全員必須にすると実務上フェーズが閉じない。
