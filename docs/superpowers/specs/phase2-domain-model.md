...
**潜在的なニーズは「語る」ことでは表出せず、具体物を介して初めて現れる**(agy指摘)。共創研究では、抽象度の高い文章を提示しても顧客は沈黙するか提示案に迎合し、具体的な試作物・シナリオといった**境界物**があって初めて共同編集が成立するとされる([Convivial Toolbox / Sanders & Stappers](https://www.bispublishers.com/convivial-toolbox.html))。

したがって `ToBe` の叩き台には、抽象的な状態記述に加えて**具体的な業務シナリオ**を含める。

```python
class ToBe(ScopedNode):
    ...
    journey_before: str = ""    # 現状の業務体験(誰が・いつ・何をしているか)
    journey_after: str = ""     # 変化後の業務体験
```

完全なモック(`build-mock`)はフェーズ2の後続だが、**Before/Afterの業務シナリオはステージ1で書ける**。「朝9時に担当者が紙の伝票を…」という具体で示すことで、顧客は「いや、実際はそこで一度FAXが入る」と訂正できる。抽象的な理想像には訂正が入らない。
...
| `internal_conflict` | 立場による実態認識の相違(経営層の見る実態 と 現場の実態) | **どちらの実態を前提にToBeを描くかを合意するための論点**。自動ではパイプラインに流さないが、**それ自体が解くべき課題なら `challenge` へ昇格できる**([ワークフローモデル](phase2-workflow-model.md) §6) |
...
**`surfaced_by` は発見経路の記録であり、確認プロセスの実施結果ではない**。「Skillが推定した関係者が1人もいない」ことは「探索しなかった」とも「探索したが追加はいなかった」とも解釈できる。確認プロセスの実施は[ワークフローモデル](phase2-workflow-model.md)の `CheckRecorded` イベントが保持する。
