...
**AsIsに内部実態(非公開情報・暗黙知)が入らないままToBeを描くと、理想の正論に終わる**。公開情報だけのAsIsからは一般論しか出ない。内部実態の充足・認識GAP・「なぜ解決していないか」の3つが、この失敗を防ぐ要になる(判断4)。
...
### 判断4: AsIsに非公開情報と暗黙知が入るまでToBeへ進まない

**記録されたAsIsに非公開情報と暗黙知が入っていないことが、ToBeを理想の正論にする。** 公開情報だけで構成されたAsIsからは、誰でも言える一般論しか導けない。現実のAsIsを可視化して共有・共感して初めて、ToBeとステップbyステップのアクション(実現のための戦略と設計)を検討できる。

**問題は現実の側ではなくドキュメントの側にある**。暗黙知は現実には常に存在しており、それが「ある」ことが問題なのではない。**我々が把握し記録したAsIsからそれが抜け落ちている**ことが問題であり、欠落したままToBeへ進むことが失敗の直接の原因になる。したがって検出すべきは「AsIsに内部実態が入っているか」であって、現実の複雑さそのものではない。
...
**`internal_as_is_missing` が判断4の中核をなす検出である**。失敗の直接の原因は「記録されたAsIsに内部実態が入っていないままToBeへ進むこと」であり、それを最も端的に捉える。公開情報だけのAsIsに対してToBeが書かれていれば、そのToBeは一般論である可能性が高い。

`next_step` は、この状態で `propose-options` へ進もうとした場合に**`hearing` へ差し戻す**(打ち手の検討に進む前に内部実態を埋める)。ただし判断5のとおり保存は拒否せず、差し戻しは推奨であって強制ではない。
...
| 要素 | 内容 | 検証方法 |
|---|---|---|
| ①**実態の共有** | **AsIsに内部実態が入っている**(公開情報だけで構成されていない)。加えて、なぜ今まで解決していないかが共有されている | 半自動(`coverage_gaps.internal_as_is_missing` が欠落を、`unverified_process` が確認プロセスの未実施を検出) |
