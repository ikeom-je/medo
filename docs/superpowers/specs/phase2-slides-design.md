...
| 5 | **叩き台の理想像**(ToBeがある場合) | 仮説としてのあるべき姿を2〜3案の振れ幅で提示。**業務シナリオのBefore/Afterを具体で示す** | `to_be`(`assumed` / `journey_before` / `journey_after`)/ `kpis` |
...
**章5では抽象論を出さない**。「業務が効率化される」ではなく「朝9時の伝票処理が、担当者の手入力からシステム取込に変わる」と具体で示す。抽象的な理想像には訂正が入らず、顧客は迎合するか沈黙する([ドメインモデル](phase2-domain-model.md))。
...
章6には、その時点で `customer` が確認者になっているチェック項目([ワークフローモデル](phase2-workflow-model.md) §6)を投影する。**チェックリストの正本はCLI側にあり、スライドはその投影**である。文書に直接書き込まない。
