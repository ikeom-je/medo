...
- リポジトリ: `ikeom-je/medo`(GitHubリモート名 `github`)
...
Run:
```bash
gh issue create --repo ikeom-je/medo \
...
Run: `gh issue close <Issue番号> --repo ikeom-je/medo --comment "遡及記録のため作成時点でclose(実装済み・マージ済み)"`
...
gh pr create --repo ikeom-je/medo --base dev --head feature/task2-storage \
...
Run: `gh pr merge --repo ikeom-je/medo --squash --delete-branch=false <PR番号>`
